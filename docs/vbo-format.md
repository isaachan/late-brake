...
本文档定义 RaceChrono Pro 导出的 VBO (VBox) 格式说明，以及到 Late Brake 内部数据格式的映射关系。
...
原始格式为 `DDMM.NNNNN`（度分格式），转换为十进制度（WGS84）：

```python
# 输入示例：+1864.768340 → 纬度 18° 64.768340'
# 分解：DD = 整数部分去掉最后两位 → 18
#       MM = 最后两位 + 小数部分 → 64.768340
# 转换：decimal_degrees = DD + MM / 60.0
# 符号：原始已经带符号，符号直接保留（负表示南纬/西经）

def convert_vbo_coord(coord: float) -> float:
    abs_coord = abs(coord)
    degrees = int(abs_coord / 100)
    minutes = abs_coord - degrees * 100
    decimal_degrees = degrees + minutes / 60.0
    return decimal_degrees if coord >= 0 else -decimal_degrees
```
